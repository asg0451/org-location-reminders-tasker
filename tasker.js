...
var PB = {};
PB = (function(tk, window, undefined) {
    "use strict";
...
// my key: o.gg2fcL1cRjcrDkUY3855wlkiJmLJWjVc

/*
  TODO: forever: watch getLoction(..), checking every x minutes. when new, get json from server, popup for current location (do hackily, with math for now: subtract gps coords, threshold)
*/
...
locations = Object.keys(orgObj);
var headers = [];
for(var i = 0; i < locations.length; i++) {
    headers.push(orgObj[locations[i]]);
}
...
for(var i = 0; i < locations.length; i++) {
    if(distanceBetween(locations[i], lastFix) < threshold) {
        sleepFor(5);
        //        popup("TODO here", headers[i].join('\n'), false, '', '', 4 /*seconds*/);
        PB.pushNote({title: 'todo', body: headers[i].join('\n')});
    }
}

function distanceBetween(coordA, coordB) {
    var lat1 = parseFloat(coordA.split(',')[0]);
    var lon1 = parseFloat(coordA.split(',')[1]);
    var lat2 = parseFloat(coordB.split(',')[0]);
    var lon2 = parseFloat(coordB.split(',')[1]);
    var R = 6371; // km
